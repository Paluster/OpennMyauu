...
public class GhosthandCheck extends Check {
    ...
    public boolean check(EntityPlayer player, PlayerDataSamples data) {
        ...
        final List<PlayerDataSamples> nearbyPlayers = getPlayersDataInAABBexcluding(player,
                ...
                p -> p != Minecraft.getMinecraft().thePlayer && p.canBeCollidedWith() && HackerDetectorEngine.isValidPlayer(p.getUniqueID()) && !p.isInvisible() && ((IEntityPlayerSamples) p).getPlayerDataSamples().posXList.size() >= MAX_TICK_DELAY);
        ...
        if (false && data.ghosthandVL.getViolationLevel() > 2) {
