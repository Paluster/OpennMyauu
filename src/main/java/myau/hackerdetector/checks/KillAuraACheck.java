...
public class KillAuraACheck extends Check {
    ...
    public boolean check(EntityPlayer player, PlayerDataSamples data) {
        ...
        final PlayerDataSamples targetData = ((IEntityPlayerSamples) data.attackInfo.target).getPlayerDataSamples();
        ...
        final List<PlayerDataSamples> nearbyPlayers = getPlayersDataInAABBexcluding(player,
                ...
                p -> p != data.attackInfo.target && p != mc.thePlayer && p.canBeCollidedWith() && HackerDetectorEngine.isValidPlayer(p.getUniqueID()) && !p.isInvisible() && ((IEntityPlayerSamples) p).getPlayerDataSamples().posXList.size() >= MAX_TICK_DELAY);
