...
public class FastbreakCheck extends Check {
        ...
        if (isCheckActive() && mc.theWorld != null && mc.thePlayer != null && mc.theWorld.isRemote) {
            for (final BrokenBlock brokenBlock : this.brokenBlocksList) {
                ...
                for (final EntityPlayer player : brokenBlock.playerList) {
                    if (player instanceof IEntityPlayerSamples) {
                ...
                final PlayerDataSamples data = ((IEntityPlayerSamples) playerBreaking).getPlayerDataSamples();
                ...
                if (breakTimeRatio < 0.95F) {
                    ...
                    if (false && data.fastbreakVL.getViolationLevel() > 6) {
    ...
    public static boolean isCheckActive() {
        return myau.module.modules.HackerDetector.isActive();
    }
