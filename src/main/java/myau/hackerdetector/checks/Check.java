...
import myau.hackerdetector.data.Vector3D;
import myau.hackerdetector.data.ViolationLevelTracker;
...
public abstract class Check implements ICheck {
    ...
    private void printFlagMessage(EntityPlayer player) {
        if (HackerDetector.get() == null) {
            return;
        }
        final String cheatName = this.getFlagType().isEmpty()
                ? this.getCheatName()
                : this.getCheatName() + " (" + this.getFlagType() + ")";
        final String formatted = ChatColors.formatColor(
                Myau.clientName + player.getDisplayName().getFormattedText() + " &eflags &c" + cheatName
        );
        ChatComponentText message = new ChatComponentText(formatted);
        message.getChatStyle().setChatHoverEvent(new HoverEvent(
                HoverEvent.Action.SHOW_TEXT,
                new ChatComponentText(EnumChatFormatting.RED + this.getCheatDescription())
        ));
    ...
    protected static List<PlayerDataSamples> getPlayersDataInAABBexcluding(Entity entity, AxisAlignedBB aabb, Predicate<? super EntityPlayer> predicate) {
        ...
        for (final EntityPlayer player : Minecraft.getMinecraft().theWorld.playerEntities) {
            if (player != entity && player.getEntityBoundingBox().intersectsWith(aabb) && predicate.test(player)) {
                list.add(((IEntityPlayerSamples) player).getPlayerDataSamples());
            }
        }
        return list;
    }
